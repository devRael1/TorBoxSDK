...
public interface IRssClient
{
    ...
    Task<TorBoxResponse> AddRssAsync(AddRssRequest request, CancellationToken ct = default);
    ...
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="request"/> is <see langword="null"/>.</exception>
    /// <exception cref="TorBoxException">Thrown when the API returns an error.</exception>
    Task<TorBoxResponse> ControlRssAsync(ControlRssRequest request, CancellationToken ct = default);
    ...
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="request"/> is <see langword="null"/>.</exception>
    /// <exception cref="TorBoxException">Thrown when the API returns an error.</exception>
    Task<TorBoxResponse> ModifyRssAsync(ModifyRssRequest request, CancellationToken ct = default);

    /// <summary>Retrieves the user's configured RSS feeds.</summary>
    /// <param name="ct">Cancellation token.</param>
    /// <returns>A list of RSS feeds.</returns>
    /// <exception cref="TorBoxException">Thrown when the API returns an error.</exception>
    Task<TorBoxResponse<IReadOnlyList<RssFeed>>> GetFeedsAsync(CancellationToken ct = default);
    ...
    /// <exception cref="TorBoxException">Thrown when the API returns an error.</exception>
    Task<TorBoxResponse<IReadOnlyList<RssFeedItem>>> GetFeedItemsAsync(long rssFeedId, CancellationToken ct = default);
}
