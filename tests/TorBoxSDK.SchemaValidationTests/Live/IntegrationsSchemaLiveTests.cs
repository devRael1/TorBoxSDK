...
/// <summary>
/// Live schema tests for the Integrations resource client.
...
[Trait("Category", "Live")]
public sealed class IntegrationsSchemaLiveTests(SchemaLiveTestFixture fixture)
{
    private readonly SchemaLiveTestFixture _fixture = fixture;

    [SkippableFact]
    public async Task GetIntegrationJobs_ResponseFields_AllMappedInSdkModel()
    {
        ...
        HttpResponseMessage response = await _fixture.HttpClient
            .GetAsync("/v1/api/integration/jobs", cts.Token)
        ...
        IReadOnlyList<string> unmapped =
            UnmappedFieldDetector.FindUnmappedFields<TorBoxResponse<IReadOnlyList<IntegrationJob>>>(json);

        // Assert
        Assert.True(
            unmapped.Count == 0,
            BuildMessage("GET /v1/api/integration/jobs", typeof(IntegrationJob), unmapped));
    }
