...
/// <summary>
/// Live schema tests for the Queued Downloads resource client.
...
[Trait("Category", "Live")]
public sealed class QueuedSchemaLiveTests(SchemaLiveTestFixture fixture)
{
    private readonly SchemaLiveTestFixture _fixture = fixture;

    [SkippableFact]
    public async Task GetQueuedDownloads_ResponseFields_AllMappedInSdkModel()
    {
        ...
        HttpResponseMessage response = await _fixture.HttpClient
            .GetAsync("/v1/api/queued/getqueued", cts.Token)
        ...
        IReadOnlyList<string> unmapped =
            UnmappedFieldDetector.FindUnmappedFields<TorBoxResponse<IReadOnlyList<QueuedDownload>>>(json);

        // Assert
        Assert.True(
            unmapped.Count == 0,
            BuildMessage("GET /v1/api/queued/getqueued", typeof(QueuedDownload), unmapped));
    }
